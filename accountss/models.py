...
import pytz
from accountss import signals
from accountss.conf import settings
from accountss.fields import TimeZoneField
from accountss.hooks import hookset
from accountss.languages import DEFAULT_LANGUAGE
from accountss.managers import EmailAdressManager, EmailConfirmationManager
from accountss.signals import signup_code_sent, signup_code_used
...
class Account(models.Model):
    ...
    language = models.CharField(
        _("language"),
        max_length=10,
        choices=settings.ACCOUNTSS_LANGUAGES,
    ...
    def create(cls, request=None, **kwargs):
        ...
        if create_email and account.user.email:
            ...
            EmailAdress.objects.add_email(account.user, account.user.email, **kwargs)
        return account
...
def user_post_save(sender, **kwargs):
    ...
    disabled = getattr(user, "_disable_account_creation", not settings.ACCOUNTSS_CREATE_ON_SAVE)
...
class EmailAdress(models.Model): # this 'EmailAdress' spelling  was done on purpose. there was conlict with another django package

    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    email = models.EmailField(max_length=254, unique=settings.ACCOUNTSS_EMAIL_UNIQUE)
    verified = models.BooleanField(_("verified"), default=False)
    primary = models.BooleanField(_("primary"), default=False)

    objects = EmailAdressManager()

    class Meta:
        verbose_name = _("email address")
        verbose_name_plural = _("email addresses")
        if not settings.ACCOUNTSS_EMAIL_UNIQUE:
    ...
    def set_as_primary(self, conditional=False):
        old_primary = EmailAdress.objects.get_primary(self.user)
        if old_primary:
            if conditional:
                return False
...
class EmailConfirmation(models.Model):
    ...
    email_address = models.ForeignKey(EmailAdress, on_delete=models.CASCADE)
